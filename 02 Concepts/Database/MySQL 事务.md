...
## 它回答的核心问题
- 它试图解释：为什么一组 SQL 不能拆成多个独立执行步骤。
- 它试图约束：业务上一致的写操作应该如何作为一个整体落地。
- 它试图区分：事务边界、ACID 性质、隔离级别和锁分别在解决什么问题。
...
## 重点部分详解
- 事务边界：边界应该围绕“一个业务闭环”来划，而不是围绕“几条 SQL”来划。
- 常见故障：把 RPC、用户输入等待、长时间计算放进事务，会放大锁持有时间和回滚代价。
- 典型代价：事务越长，undo 保留越久，锁等待和并发冲突概率越高。
- 实战判断：如果一组操作失败后不能接受任何部分生效，就应该认真考虑事务；反之不要滥用事务。
